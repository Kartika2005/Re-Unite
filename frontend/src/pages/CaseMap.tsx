...
export function CaseMap() {
  ...
  const statCards = [
    ...
    {
      label: "Reported",
      value: cases.filter((c) => c.status === "REPORTED").length,
      icon: FileText,
      color: "text-amber-500",
    },
    {
      label: "Scanning",
      value: cases.filter((c) => c.status === "SCANNING").length,
      icon: ScanLine,
      color: "text-violet-500",
    },
    {
      label: "Found",
      value: cases.filter((c) => c.status === "FOUND").length,
      icon: CheckCircle,
        ...
        <CardContent className="pt-4 pb-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {userLocation && (
              <Badge
                variant="outline"
                className="bg-sky-50 text-sky-700 border-sky-200 text-sm font-semibold"
              >
                {nearbyCases.length} case{nearbyCases.length !== 1 ? "s" : ""}{" "}
                within {radius} km
              </Badge>
            )}
          </div>
              ...
              >
                <Popup>
                  <div className="text-sm leading-relaxed">
                    <strong>{c.name || "Aadhaar Report"}</strong>
                    <br />
